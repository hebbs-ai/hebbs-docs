...
Upload a single file with path control. The `remote_path` determines where the file is stored in the workspace vault.

```python
await hb.upload("./meeting.md", remote_path="entities/acme/meeting.md")
```

#### `file_status(remote_path)`
...
```python
status = await hb.file_status("entities/acme/meeting.md")
