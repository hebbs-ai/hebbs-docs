...
Upload a single file with path control using the SDK:

**Python:**
```python
await hb.upload("./meeting.md", remote_path="entities/acme/meeting-2026-04-16.md")
```

**TypeScript:**
```typescript
const content = new Uint8Array(await fs.readFile("./meeting.md"));
await hb.upload("entities/acme/meeting-2026-04-16.md", content);
```

**REST API:**
```bash
curl -X POST .../v1/upload \
  -H "Authorization: Bearer hb_live_sk_..." \
  -F "files=@meeting.md;filename=entities/acme/meeting-2026-04-16.md"
```

The `remote_path` controls exactly where the file lands in the workspace vault. Directories are created automatically. Only the uploaded file is indexed; existing files are not re-processed.

## File Status

Check whether a specific file has been indexed:

**Python:**
```python
status = await hb.file_status("entities/acme/meeting-2026-04-16.md")
# {"status": "indexed", "sections": 3, "sections_synced": 3, "memories": 8}
```

**TypeScript:**
```typescript
const status = await hb.fileStatus("entities/acme/meeting-2026-04-16.md");
```

**REST API:**
```bash
curl .../v1/files/entities/acme/meeting-2026-04-16.md/status \
  -H "Authorization: Bearer hb_live_sk_..."
```
...
| Status | Meaning |
|---|---|
| `pending` | File uploaded, indexing not started yet |
| `indexing` | LLM extraction or embedding in progress |
| `indexed` | All sections synced, memories created |
| `not_found` | File does not exist in the workspace |

**Poll after upload** to know when a file is ready:

```python
await hb.upload("./report.md", remote_path="entities/acme/report.md")

while True:
    status = await hb.file_status("entities/acme/report.md")
...
Remove a file and forget its memories:
...
**TypeScript:**
```typescript
await hb.deleteFile("entities/acme/old-notes.md");
```

**REST API:**
```bash
curl -X DELETE .../v1/files/entities/acme/old-notes.md \
  -H "Authorization: Bearer hb_live_sk_..."
```

The file is removed from disk and the engine forgets associated memories during the next indexing cycle.
